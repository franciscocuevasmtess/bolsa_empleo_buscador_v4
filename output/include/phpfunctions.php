...
try {
    // Establece la fecha de fin (evento futuro)
    $fecha_fin_str = isset($data["fecha_expiracion_vacancia"]) ? htmlspecialchars($data["fecha_expiracion_vacancia"], ENT_QUOTES, 'UTF-8') : '2024-12-31 23:59:59';
    $fecha_fin = new DateTime($fecha_fin_str);

    // Verifica que la fecha de fin es posterior a la fecha actual
    if ($fecha_fin < $fecha_actual) {
        throw new Exception("La fecha de fin debe ser posterior a la fecha actual.");
    }

    // Calcula la diferencia entre la fecha actual y la fecha de fin
    $diferencia = $fecha_actual->diff($fecha_fin);

    // Almacena los componentes de la diferencia en variables separadas
    $días = $diferencia->days;
    $horas = $diferencia->h;
    $minutos = $diferencia->i;

    // Genera HTML con cada parte en su propia caja
    $html = <<<HTML
<!DOCTYPE html>
<html lang="es">
<head>
...
$formattedDate = strftime('%A %d de %B %Y', $date->getTimestamp()); // Imprimir el resultado 
...
if($value<1){
	$value=$value." Vacancias";
		...
		return $value;
				...
				if($table=="bolsa_empleo.vacancia" && $field=="id_estado" && $ptype=="list")
	{
		if ($data["id_estado"] == '1') {  
...
$value= $formattedDate;;
				...
				if($table=="bolsa_empleo.vacancia" && $field=="fk_id_feria_empleo")
	{
		...
		return $value;
				...
				if($table=="bolsa_empleo.vacancia" && $field=="regresi")
	{
		if ($data1 != null) {
   $limitDate = $data1['fecha_expiracion_vacancia']; // Recuperar la última fecha insertada
}
...
.tick {
  ...
  display: inline-flex; /* Utilizamos inline-flex para que el contenedor se ajuste al contenido */
...
.tick-group {
  margin: 0 0.1em; /* Ajustamos el espacio entre grupos */
}

.tick-label {
  font-size: 10px; /* Tamaño de fuente de la etiqueta */
  color: #3c3e3c; /* Color de la etiqueta */
  margin-left: 0.2em; /* Espaciado a la izquierda */
}
...
@media (max-width: 600px) {
  ...
  .tick-flip {
      font-size: 8px; /* Tamaño de fuente del contador para dispositivos móviles */
  }

  .tick-label {
      font-size: 10px; /* Tamaño de fuente de la etiqueta para dispositivos móviles */
  }
}
</style>
...
$value= $html;;
		return $value;
				...
				if($table=="eportal.persons" && $field=="sexo")
	{
		if ($data["sexo"] == 'F') {  
     $value="Femenino";  
}
...
$value="<span style='color: " . $color . "'>" .$value . "</span>"; ;
		return $value;
				...
				if($table=="bolsa_empleo.cvc_programas" && $field=="fk_id_programas" && $ptype=="list")
	{
		$sql = DB::PrepareSQL("select nombre from bolsa_empleo.programas where id_programas = ':1'", $data["fk_id_programas"]);
$rs = DB::Query($sql);
$dataf = $rs->fetchAssoc();
$value = $dataf["nombre"];
		return $value;
	}
				if($table=="bolsa_empleo.vista_curriculum_cvc" && $field=="sexo")
	{
		if ($data["sexo"] == 'F') {  
     $value="Femenino";  
}
...
$value="<span style='color: " . $color . "'>" .$value . "</span>"; ;
		return $value;
				...
				if($table=="detalle_vacancia" && $field=="fecha_expiracion_vacancia")
	{
		setlocale(LC_TIME, 'es_VE.UTF-8','esp'); // Crear un objeto DateTime
...
$value = utf8_encode($formattedDate);;
		return $value;
				...
				if($table=="detalle_vacancia" && $field=="id_estado" && $ptype=="list")
	{
		if ($data["id_estado"] == '1') {  
     $value="<span class='badge badge-pendiente'>" .$value . "</span>"; 
}
...
if ($data["id_estado"] == '6') {  
     $value="<span class='badge badge-cerrada'>" .$value . "</span>";  
		...
		return $value;
				...
				if($table=="detalle_vacancia" && $field=="fecha_creacion_vacancia")
	{
		setlocale(LC_TIME, 'es_VE.UTF-8','esp'); // Crear un objeto DateTime
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="eportal.persons" && $field=="esindigena")
	{
		return 1;
	}
				if($table=="eportal.persons" && $field=="multiselect_discapacidades")
	{
		return 1;
	}
				...
				if($table=="bolsa_empleo.cvc_idiomas" && $field=="fk_personaid")
	{
		return $_SESSION["persona_id"];
				...
				if($table=="bolsa_empleo.cvc_programas" && $field=="fk_personaid")
	{
		return $_SESSION["persona_id"];
	}
				...
				if($table=="bolsa_empleo.cvc_experiencia_laboral" && $field=="proveedor" && $ptype=="edit")
	{
		return 'Usuario';
				...
				if($table=="bolsa_empleo.cvc_experiencia_laboral" && $field=="proveedor" && $ptype=="add")
	{
		return 'Usuario';
	}
				if($table=="bolsa_empleo.cvc_experiencia_laboral" && $field=="proveedor" && $ptype=="search")
	{
		return 'Usuario';
	}
				if($table=="vista_para_curri_experiencia_laboral" && $field=="fk_persona_id")
	{
		return $_SESSION["persona_id"];
				...
				if($table=="vista_curri_estudios_realizados" && $field=="fk_personaid")
	{
		return $_SESSION["persona_id"];
	}
				if($table=="vista_curri_programas_realizados" && $field=="fk_personaid")
	{
		return $_SESSION["persona_id"];
	}
				if($table=="vista_curri_idiomas" && $field=="fk_personaid")
	{
		return $_SESSION["persona_id"];
	}
				if($table=="vista_curri_telefonos" && $field=="person_id")
	{
		return $_SESSION["persona_id"];
	}
				if($table=="vista_curri_referencia_laborales" && $field=="id_persona")
	{
		return $_SESSION["persona_id"];
	}
